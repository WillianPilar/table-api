...
public class TableServiceImpl implements TableService {
	...
	public List<Integer> processNumbersToList(String numbers) {
		...
		/*
		 * Retorna a quantidade de ',' Quantidade de ',' + 1 = Quantidade de números na
		 * Requisição
		 */
		...
		for (int i = 0; i < numberAux; i++) {
			...
			/*
			 * Utiliza ',' para validar se existe mais de 1 número para ser processado Caso
			 * contrário processa o número restante
			 */
			if (numbers.contains(",")) {

				/*
				 * Valida o index da próxima ',' Captura o numéro antes da ',' Remove da lista
				 * de números da entrada Adiciona na lista de números para processamento
				 */
	...
	public int[][] processFullMatrix(int turns, int[][] matrizProcess, int columnAndLine) {
		...
		int indexLineAndColumn = 1;
		
		/* Processa a parte interna da Matriz */
		for (int turn = 0; turn < turns; turn++) {
			
			/* Gera uma nova lista com os valores que não foram rotacionados */
			for (int line = indexLineAndColumn; line < columnAndLine - turn; line++) {
				for (int column = indexLineAndColumn; column < columnAndLine - turn; column++) {
					if (!(line <= turn || line >= lastColumnAndLine - turn) && !(column <= turn || column >= lastColumnAndLine - turn)) {
						listNumbersProcess.add(matrizProcess[line][column]);
					}
				}
			}
			
			/* Processa uma Matriz com os valores que não foram rotacionados
			 * Gera uma lista rotacionada a partir da matriz
			 * Move os números nessa nova Matriz */
			...
			/* Salva os números internos que foram rotacionados na Matriz principal */
			matrizProcess = this.updateMatriz(matrizProcess, matriz, turn + 1, columnAndLine - 1);
			
			/* Zera a Lista de números que não foram rotacionados - Pois os mesmo foram rotacionados
			 * Soma 1 na linha e coluna que devem ser rotacionados no próximo turno */
			listNumbersProcess = new ArrayList<Integer>();
			indexLineAndColumn++;
	...
	private int[][] updateMatriz(int[][] matriz, int[][] matrizAux, int index, int lastColumnAndLine) {
		...
		/* Substitui os valores da Matriz principal que não foram rotacionados com os valores da nova matriz rotacionada */
		for (int line = index; line < lastColumnAndLine; line++) {

			for (int column = index; column < lastColumnAndLine; column++) {

				if ((line >= index && line <= lastColumnAndLine - index)
						&& (column >= index && column <= lastColumnAndLine - index)) {

					if ((line == index || line == lastColumnAndLine - index)
							|| (column == index || column == lastColumnAndLine - index)) {

						matriz[line][column] = matrizAux[line - index][column - index];
					}
				}
